...
function extractAllToolsetsFromRegistry() {
  const indexPath = join(SRC, "registry", "index.ts");
  const content = read(indexPath);
  const block = content.match(/const ALL_TOOLSETS[\s\S]*?=\s*\[([\s\S]*?)\];/);
  if (!block) {
    addError("src/registry/index.ts — could not parse ALL_TOOLSETS array");
    return new Set();
  }
  const registered = new Set();
  for (const line of block[1].split("\n")) {
    const m = line.match(/(\w+Toolset)/);
...
function checkZodDescribeInToolHandlers() {
  for (const handler of ALLOWED_TOOL_HANDLERS) {
    const file = join(SRC, "tools", handler);
    const content = read(file);
    const schemaBlocks = content.match(/inputSchema:\s*\{[\s\S]*?\n\s*\}/g) ?? [];
    for (const block of schemaBlocks) {
      const zodChains = block.match(/z\.\w+\([^)]*\)(?:\.\w+\([^)]*\))*/g) ?? [];
      for (const chain of zodChains) {
        if (!chain.includes(".describe(") && !chain.includes("...")) {
          addWarning(`${handler} — Zod field may be missing .describe(): ${chain.slice(0, 60)}...`);
        }
      }
    }
  }
...
function main() {
  ...
  // Zod .describe() coverage is advisory — many fields use shared schemas.
  // checkZodDescribeInToolHandlers();
