...
// CreateLicenseClientForAccount creates a license client for a specific account ID
// This is useful when the account ID differs from the config's default account ID
// It reuses the same client instance as CreateLicenseClient
func (f *ClientFactory) CreateLicenseClientForAccount(ctx context.Context, accountID string) (*license.CustomLicensesApiService, error) {
	// Just use the same client instance
	return f.CreateLicenseClient(ctx)
}

// GetConfig returns the configuration used by this factory
func (f *ClientFactory) GetConfig() *config.Config {
	return f.config
}

// GetLogger returns the logger used by this factory
func (f *ClientFactory) GetLogger() *slog.Logger {
	return f.logger
}
