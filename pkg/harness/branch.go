...
func CreateBranchTool(config *config.Config, client *client.BranchService) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("create_branch",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			data, err := client.Create(ctx, scope, repoIdentifier, createBranchReq)
			if err != nil {
...
// CommitFileTool creates a tool for committing a file change to a branch
func CommitFileTool(config *config.Config, client *client.BranchService) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("commit_file",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			commitFileReq := &dto.CommitFileRequest{
				Branch:    branch,
				FilePath:  filePath,
				Content:   content,
				CommitMsg: commitMsg,
...
// CommitMultipleFilesTool creates a tool for committing multiple file changes in a single commit
func CommitMultipleFilesTool(config *config.Config, client *client.BranchService) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("commit_multiple_files",
		...
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			files := make([]dto.CommitFileOp, 0, len(filesData))
			for _, fileRaw := range filesData {
				...
				files = append(files, dto.CommitFileOp{
					FilePath:  filePath,
					Content:   content,
					IsNewFile: isNewFile,
				})
			...
			commitReq := &dto.CommitMultipleFilesRequest{
				Branch:    branch,
				CommitMsg: commitMsg,
				Files:     files,
			}
