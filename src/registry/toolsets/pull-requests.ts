...
interface FlatBodyField {
  wire: string;
  aliases?: readonly string[];
}

const PR_MERGE_BODY_FIELDS: readonly FlatBodyField[] = [
  { wire: "method" },
...
const PR_REVIEW_SUBMIT_BODY_FIELDS: readonly FlatBodyField[] = [
  { wire: "decision" },
  { wire: "commit_sha", aliases: ["commitSha", "source_sha", "sourceSha"] },
];
...
/**
 * harness_execute merges params into the top-level input, so action options
 * arrive either in body or flattened next to repo_id/pr_number. Collect both
 * shapes into the wire body so required-field validation sees them.
 */
function collectFlatBody(
  input: Record<string, unknown>,
  fields: readonly FlatBodyField[],
  action: string,
  ...
  for (const field of fields) {
    ...
    if (bodyValue && inputValue && !Object.is(bodyValue.value, inputValue.value)) {
      throw new Error(
        `Conflicting ${action} values for "${field.wire}" between ` +
...
function pullRequestMergeBody(input: Record<string, unknown>): Record<string, unknown> {
  return collectFlatBody(input, PR_MERGE_BODY_FIELDS, "pull_request.merge");
}

function prReviewSubmitBody(input: Record<string, unknown>): Record<string, unknown> {
  return collectFlatBody(input, PR_REVIEW_SUBMIT_BODY_FIELDS, "pr_reviewer.submit_review");
}
  ...
  resources: [
    {
      ...
      executeActions: {
        ...
        merge: {
          ...
          paramsSchema: REPO_PR_PARAMS,
          actionDescription:
            "Merge a pull request. Body fields: method (merge/squash/rebase/fast-forward), source_sha (required — GET the PR and use its source_sha), delete_source_branch (boolean), dry_run (boolean), dry_run_rules (boolean), message, title, bypass_rules (boolean), bypass_message.",
          bodySchema: {
            ...
            fields: [
              ...
              { name: "source_sha", type: "string", required: true, description: "Expected source SHA for optimistic locking. GET the pull request first and pass its source_sha — the merge is rejected if a newer commit landed." },
    ...
    },
    {
      ...
      executeActions: {
        submit_review: {
          ...
          bodyBuilder: prReviewSubmitBody,
          responseExtractor: passthrough,
          paramsSchema: REPO_PR_PARAMS,
          actionDescription:
            "Submit a review decision. Body fields: decision (required — 'approved', 'changereq', or 'reviewed'), commit_sha (required — SHA reviewed against; GET the pull request and use its source_sha).",
          bodySchema: {
            ...
            fields: [
              ...
              { name: "commit_sha", type: "string", required: true, description: "Commit SHA reviewed against. GET the pull request first and pass its source_sha." },
            ],
          },
        },
      },
