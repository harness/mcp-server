...
const providerExtract = (raw: unknown): unknown => raw;
...
const variableSetItemPath = (
  ...
  if (!id) {
    throw new Error(
      'Missing required field "variable_set_id" for infrastructure_variable_set. Pass it via params or as resource_id.',
    );
  }
  return `${variableSetCollectionPath(input, config)}/${encodeURIComponent(id)}`;
};

const providerCreatePath = (input: Record<string, unknown>): string => {
  const body = (input.body as Record<string, unknown> | undefined) ?? {};
  const type = (input.type as string | undefined) ?? (body.type as string | undefined);
  if (!type) {
    throw new Error(
      'Missing required field "type" for infrastructure_provider create. Pass it in body.type (e.g. "aws", "terraform").',
    );
  }
  return `/iacm/api/providers/${encodeURIComponent(type)}`;
};

const providerCreateBody = (input: Record<string, unknown>): Record<string, unknown> => {
  const body = { ...((input.body as Record<string, unknown> | undefined) ?? {}) };
  delete body.type;
  return body;
};

const providerVersionMethod = (input: Record<string, unknown>): "POST" | "PUT" =>
  input.version ? "PUT" : "POST";

const providerVersionPath = (input: Record<string, unknown>): string => {
  const id = input.id as string | undefined;
  if (!id) {
    throw new Error(
      'Missing required field "id" for infrastructure_provider update. Pass the provider id via params or as resource_id.',
    );
  }
  if (input.version) {
...
const providerVersionBody = (input: Record<string, unknown>): Record<string, unknown> => {
  const body = { ...((input.body as Record<string, unknown> | undefined) ?? {}) };
  if (input.version) {
    delete body.version;
  } else if (body.version === undefined || body.version === "") {
    throw new Error(
      'Missing required field "version" for infrastructure_provider version create. ' +
...
const providerCreateSchema: BodySchema = {
  ...
  fields: [
    {
      ...
      required: false,
      description:
        "REQUIRED (enforced by pathBuilder). Provider type path segment (e.g. aws, terraform). Stripped from the JSON body.",
    },
    { name: "description", type: "string", required: false, description: "Optional provider description" },
  ],
};

const providerUpdateSchema: BodySchema = {
  description:
...
export const infrastructureToolset: ToolsetDefinition = {
  ...
  description:
    ...
    "Use infrastructure_workspace to list, get, create, or update workspaces; infrastructure_variable_set for reusable " +
    "variable sets (account/org/project); infrastructure_resource for Terraform resources and outputs; " +
    "infrastructure_module (account/org/project) and infrastructure_provider for private registries; infrastructure_workspace_costs for cost breakdown; " +
    "and infrastructure_activity_resource_change for activity diffs.",
  optIn: false,
  resources: [
    // ─── Workspace ─────────────────────────────────────────────────────────
    {
      resourceType: "infrastructure_workspace",
      aliases: ["iacm_workspace"],
      searchAliases: ["iacm"],
      ...
      description:
        ...
        "See also: infrastructure_resource for Terraform resources, infrastructure_workspace_costs for cost breakdown.",
      ...
      operations: {
        ...
        create: {
          ...
          description:
            ...
            "(resource_type=infrastructure_workspace, workspace_id=<identifier>) to fetch the created workspace.",
        },
        update: {
          method: "PUT",
          path: "/iacm/api/orgs/{org}/projects/{project}/workspaces/{workspaceId}",
          ...
          description:
            ...
            "Response is { policy_evaluation } only — not the workspace. Follow up with harness_get " +
            "(resource_type=infrastructure_workspace, workspace_id=<identifier>) to fetch the updated workspace.",
        },
      },
    },
    ...
    // ─── Variable Sets ─────────────────────────────────────────────────────
    {
      resourceType: "infrastructure_variable_set",
      aliases: ["iacm_variable_set"],
      searchAliases: ["iacm"],
      ...
      description:
        ...
        "See also: infrastructure_workspace.variable_sets for attaching sets to a workspace.",
      toolset: "infrastructure",
      scope: "project",
    ...
    // ─── Terraform Resources / Outputs / DataSources ───────────────────────
    {
      ...
      description:
        ...
        "workspace_id is required — use harness_list with infrastructure_workspace to find workspace identifiers.",
      toolset: "infrastructure",
      scope: "project",
      identifierFields: ["workspace_id"],
      listFilterFields: [
        {
          ...
          required: true,
          description:
            "The workspace identifier (use harness_list with infrastructure_workspace to find it). " +
    ...
    // ─── Module Registry ───────────────────────────────────────────────────
    {
      resourceType: "infrastructure_module",
      aliases: ["iacm_module"],
      searchAliases: ["iacm"],
      ...
      operations: {
        ...
        get: {
          ...
          description:
            ...
            "Always call harness_list on infrastructure_module first to get the id, then call harness_get. " +
    ...
    // ─── Provider Registry ─────────────────────────────────────────────────
    {
      resourceType: "infrastructure_provider",
      aliases: ["iacm_provider"],
      searchAliases: ["iacm"],
      ...
      description:
        ...
        "(unlike infrastructure_module). Ambient org_id/project_id and HARNESS_ORG/HARNESS_PROJECT are ignored. " +
      ...
      operations: {
        ...
        create: {
          ...
          description:
            ...
            "(resource_type=infrastructure_provider, id=<id>) or harness_list before version updates. " +
          ...
          paramsSchema: {
            fields: [
              ...
              },
              {
                ...
                required: false,
                description:
                  "When set, updates that existing version via PUT. When omitted, creates a new version via POST (include body.version).",
              },
            ],
          },
          description:
    ...
    // ─── Workspace Costs ───────────────────────────────────────────────────
    {
      ...
      description:
        ...
        "See also: infrastructure_workspace for workspace metadata, infrastructure_resource for resource-level cost_data.",
      toolset: "infrastructure",
      scope: "project",
      identifierFields: ["workspace_id"],
      listFilterFields: [
        {
          name: "workspace_id",
          required: true,
          description: "The workspace identifier. Use harness_list with infrastructure_workspace to find it.",
          type: "string",
        },
      ],
      relatedResources: [
    ...
    {
      resourceType: "infrastructure_activity_resource_change",
      aliases: ["iacm_activity_resource_change"],
      searchAliases: ["iacm"],
