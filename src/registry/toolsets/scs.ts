    ...
    // ── Remediation Pull Requests (P3-6: PR creation + tracking) ──────
    {
      ...
      operations: {
        ...
        create: {
           ...
           *   • HTTP 5xx / network / non-HarnessApiError ⇒ fail OPEN (return
           *                 without blocking). The check is best-effort and
          ...
          preflight: async ({ client, input, registry, signal }) => {
            ...
            for (let page = 0; page < PREFLIGHT_MAX_PAGES; page++) {
              ...
              try {
                ...
                //   5xx / network / timeout → fail OPEN. The check is best-effort;
                //         transient upstream issues should not permanently block
                //         remediation creation (toolsets do not log — handlers own observability).
