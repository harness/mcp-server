...
export const presignedDownloadExtract = (raw: unknown): Record<string, unknown> => {
  ...
  out._display_hint = PRESIGNED_DOWNLOAD_HINT;
  return out;
};
...
export const attestationListExtract = (raw: unknown): {
  items: unknown[];
  total: number;
...
export const attestationDetailsExtract = (raw: unknown): Record<string, unknown> => {
  if (!isRecord(raw)) return {};
  const exec = asRecord(raw.execution_context);
