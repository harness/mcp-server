  ...
  server.registerPrompt(
    ...
    async ({ featureFlagName, workspaceId, orgId, projectId }) => {
      ...
      const nativeModeCaveat = workspaceId
        ? ""
        : "\n\nNote: in Harness-native mode (org_id/project_id), feature_flag_rollout_status is not yet implemented server-side and will error — step 4 only works today with workspace_id (legacy mode). Steps 3 and 7 work with org_id+project_id.";
...
Steps:
1. **Get flag details**: Call harness_get with resource_type="feature_flag", feature_flag_name="${featureFlagName}", ${scopeArgs} to see the current flag state
2. **List environments**: Call harness_list with resource_type="feature_flag_environment", ${scopeArgs} to see available environments
3. **Get flag definition per environment**: For each environment, call harness_get with resource_type="feature_flag_definition", feature_flag_name="${featureFlagName}", ${scopeArgs}, environment_id=<env_id> to see treatments and rules
4. **Check rollout statuses**: Call harness_list with resource_type="feature_flag_rollout_status", ${scopeArgs} for rollout status context
...
Present the rollout plan for review. Use harness_execute with resource_type="feature_flag", action="kill" or action="restore", ${scopeArgs}, feature_flag_name="${featureFlagName}", environment_id=<env_id> to execute each phase after user approval.`,
          },
        }],
      };
    },
