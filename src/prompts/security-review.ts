...
Steps:
1. Call harness_list with resource_type="application_security_issue"${projectId ? `, project_id="${projectId}"` : ""}, severity="${severityFilter}" to get current security findings
2. Call harness_list with resource_type="application_security_artifact"${projectId ? `, project_id="${projectId}"` : ""} to check artifact-level security posture
3. Call harness_list with resource_type="application_security_code_repo"${projectId ? `, project_id="${projectId}"` : ""} to check code repository security
