...
const RESOURCE_SEGMENTS: Record<string, { type: string; contextField: ContextField }> = {
  ...
  "infrastructures":  { type: "infrastructure_definition", contextField: "resource_id" },
  "infrastructure-definitions": { type: "infrastructure_definition", contextField: "resource_id" },
  ...
  "splits":           { type: "feature_flag",          contextField: "resource_id" },
  ...
  "issues":           { type: "security_issue",           contextField: "resource_id" },
  "exemptions":       { type: "security_exemption",       contextField: "resource_id" },
 ...
 *
 * Both the old (`fme_*`) and renamed canonical (`feature_flag_*`) names are listed:
 * this Set is matched against raw caller input (resource_type) before the registry's
 * alias layer resolves it, so a caller passing either name must be recognized.
 */
const FME_HARNESS_NATIVE_ONLY_RESOURCE_TYPES = new Set([
  "fme_segment",
  "fme_segment_definition",
  "feature_flag_segment",
  "feature_flag_segment_definition",
]);
...
export function applyUrlDefaults(
  ...
  // Harness-native-only resources (feature_flag_segment/_definition, old fme_segment*)
  // are excluded: they have no workspace_id contract, so a stray value must not suppress
  // org/project. Match both the old `fme_` prefix and the renamed `feature_flag` prefix,
  // since this runs on raw caller input before the registry's alias layer resolves it.
  const declaredResourceType = (args.resource_type as string | undefined) ?? parsed.resource_type;
  const isFeatureFlagType =
    declaredResourceType?.startsWith("fme_") === true ||
    declaredResourceType?.startsWith("feature_flag") === true;
  const hasWorkspaceId = typeof args.workspace_id === "string" && args.workspace_id !== "";
  const skipOrgProjectFromUrl =
    hasWorkspaceId &&
    isFeatureFlagType &&
    !FME_HARNESS_NATIVE_ONLY_RESOURCE_TYPES.has(declaredResourceType as string);
