...
import { mergeConfigWithHttpRequest } from "./utils/http-session-config.js";
...
async function startHttp(config: Config, port: number): Promise<void> {
  ...
  app.use((_req, res, next) => {
    ...
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, mcp-session-id, x-harness-pipeline-version, x-api-key, harness-account, authorization");
  ...
  app.post("/mcp", async (req, res) => {
    ...
    try {
      const sessionConfig = mergeConfigWithHttpRequest(config, req);
