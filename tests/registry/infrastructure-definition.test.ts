/**
 * CD infrastructure_definition wiring (lives in the environments toolset) —
 * ensure create/update bodyBuilders synthesize non-empty yaml (NG contract) and
 * pick up tool-level org_id/project_id before yaml synthesis so scope fields
 * appear inside body.yaml.
 *
...
import { environmentsToolset } from "../../src/registry/toolsets/environments.js";
import { Registry } from "../../src/registry/index.js";
import type { Config } from "../../src/config.js";
import type { HarnessClient } from "../../src/client/harness-client.js";

const infra = environmentsToolset.resources.find((r) => r.resourceType === "infrastructure_definition");
if (!infra) throw new Error("infrastructure_definition resource missing from environments toolset");
...
describe("infrastructure deep links", () => {
  ...
  it("list: openInHarness is environment details with the Infrastructure section", async () => {
    ...
    const result = (await registry.dispatch(client, "infrastructure_definition", "list", {
      org_id: "default",
      project_id: "avi",
  ...
  it("get: openInHarness uses environmentRef from the response", async () => {
    ...
    const result = (await registry.dispatch(client, "infrastructure_definition", "get", {
  ...
  it("create: openInHarness uses environmentRef when environment_id is not a query param", async () => {
    ...
    const result = (await registry.dispatch(client, "infrastructure_definition", "create", {
      org_id: "default",
      project_id: "avi",
      body: {
        identifier: "k8s",
