...
describe("Registry", () => {
  ...
  describe("FME feature flag list — Split lifecycle status filter", () => {
    it("pages through the workspace and returns only flags with matching status", async () => {
      const registry = new Registry(
        makeConfig({
          HARNESS_TOOLSETS: "feature-flags",
          HARNESS_FME_API_KEY: "fme-test-key",
        }),
      );
      const calls: unknown[] = [];
      const client = makeClient(
        vi.fn().mockImplementation(async (opts: { params?: Record<string, unknown> }) => {
          calls.push(opts);
          ...
          return {
            objects: [{ name: "c", status: "ARCHIVED" }],
            offset: 2,
            limit: 2,
            totalCount: 3,
          };
      ...
      const result = (await registry.dispatch(client, "fme_feature_flag", "list", {
        ...
        size: 2,
        status: "ARCHIVED",
      })) as { objects: { name: string; status: string }[]; totalCount: number };

      expect(result.objects).toHaveLength(2);
      expect(result.objects.map((o) => o.name).sort()).toEqual(["b", "c"]);
      expect(result.totalCount).toBe(2);
      expect(calls).toHaveLength(2);
      const second = calls[1] as { params?: Record<string, unknown> };
      expect(second.params?.offset).toBe(2);
      expect(second.params?.limit).toBe(2);
    });

    it("does not page when status filter is omitted", async () => {
      const registry = new Registry(
        makeConfig({
          HARNESS_TOOLSETS: "feature-flags",
          HARNESS_FME_API_KEY: "fme-test-key",
