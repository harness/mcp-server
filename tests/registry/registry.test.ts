...
describe("Registry", () => {
  ...
  describe("methodBuilder and input-aware responseExtractor", () => {
    ...
    it("cost_recommendation_filter list returns empty values for unexpected response shape", async () => {
      const mockRequest = vi.fn().mockResolvedValue({ status: "SUCCESS", data: null });
      const client = makeClient(mockRequest);
    ...
    it("cost_recommendation_filter get normalizes object responses with costBuckets array", async () => {
      const mockRequest = vi.fn().mockResolvedValue({
        data: { costBuckets: ["Platform", "Security"] },
      });
      const client = makeClient(mockRequest);
      ...
      expect(result).toEqual({ values: ["Platform", "Security"] });
    });
  });
