...
describe("aitToolset structure", () => {
  ...
  it("registers AIT test and knowledge-base resource types", () => {
    ...
    expect(types).toContain("kb_crawl");
    expect(types).toContain("kb_crawl_page");
    expect(types).toContain("kb_page_artifact");
...
describe("ait opt-in with Registry", () => {
  ...
  it("IS present when explicitly enabled with HARNESS_TOOLSETS=+ait", () => {
    ...
    expect(registry.getAllResourceTypes()).toContain("kb_crawl");
  });
