...
describe("gitops_agent", () => {
  ...
  it("delete: resource_id → DELETE /gitops/api/v1/agents/{agentIdentifier}", async () => {
    const mockRequest = vi.fn().mockResolvedValue({});
    const client = makeClient(mockRequest);
  ...
  it("delete: fails fast when resource_id is missing (paramsSchema)", async () => {
...
describe("gitops_application", () => {
  ...
  it("delete: throws when required delete params are missing (paramsSchema)", async () => {
    const client = makeClient(vi.fn());
  ...
  it("delete: throws when cascade is not provided but finalizers param is present", async () => {
    const client = makeClient(vi.fn());
