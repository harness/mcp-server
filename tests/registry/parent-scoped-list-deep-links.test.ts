/**
 * Parent-scoped list deep links must keep the parent id from the request
 * (via pipelineScopedListExtract) — not overwrite with item.identifier (the child id).
 */
...
describe("input_set list deep links", () => {
  let registry: Registry;
  ...
  it("keeps request pipeline_id in openInHarness (not item.identifier)", async () => {
    const mockRequest = vi.fn().mockResolvedValue({
      data: {
        content: [
          { identifier: "my_input_set", name: "My Input Set" },
        ],
        totalElements: 1,
      },
    });
    ...
    const link = result.items[0]!.openInHarness as string;
    expect(link).toContain("/pipelines/parent_pipeline/input-sets");
    expect(link).not.toContain("/pipelines/my_input_set/");
  });
});
