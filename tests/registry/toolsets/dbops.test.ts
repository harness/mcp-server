...
describe("database_execute_llm_authoring_pipeline endpoint spec", () => {
  ...
  it("documents the correct NG setting key dbops_llm_authoring_pipeline (not _id suffix)", () => {
    const description = executeResource.description ?? "";
    const pipelineIdField = runAction.bodySchema?.fields?.find(
      (f) => f.name === "pipeline_identifier" || f.name === "pipelineIdentifier",
    );
    const fieldDescription = pipelineIdField?.description ?? "";

    expect(description).toContain("dbops_llm_authoring_pipeline");
    expect(description).not.toContain("dbops_llm_authoring_pipeline_id");
    expect(fieldDescription).toContain("dbops_llm_authoring_pipeline");
    expect(fieldDescription).not.toContain("dbops_llm_authoring_pipeline_id");
  });
});
