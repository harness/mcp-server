...
const TOOLSET_COMPANION_FILES = new Set(["chaos-descriptions.ts"]);
...
describe("coding standards — logging and HTTP", () => {
  ...
  it("does not call fetch() from tool handlers or toolsets (except schema discovery)", () => {
    const violations: string[] = [];
    const allowedFetchPaths = [
      "src/tools/entity-schema/",
      "src/tools/harness-schema.ts", // live entity schema fallback via LiveSchemaFetcher
    ];
    const dirs = [TOOLS_DIR, TOOLSETS_DIR];
    for (const dir of dirs) {
      for (const file of walkTsFiles(dir)) {
        const relPath = rel(file);
        if (allowedFetchPaths.some((allowed) => relPath.startsWith(allowed) || relPath === allowed)) {
          continue;
        }
    ...
    expect(violations, `fetch() in tools/toolsets:\n${violations.join("\n")}`).toEqual([]);
  });
});
