...
const FORBIDDEN_TOOLSET_IMPORTS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  { pattern: /from\s+["'][^"']*\/utils\/logger/, reason: "createLogger import — toolsets are pure data; log in handlers" },
];
...
describe("Coding standards — logging and HTTP", () => {
  ...
  it("toolset files do not use console.* or createLogger (stderr logging belongs in handlers)", () => {
    const violations: string[] = [];
    ...
    for (const file of walkTsFiles(toolsetDir)) {
      ...
      if (/\bconsole\.(log|error|warn|info|debug)\s*\(/.test(content)) {
        violations.push(`${fileRel}: console.* call`);
      }
