...
describe("Coding standards — documentation consistency", () => {
  ...
  it("docs/coding-standards.md documents global fetch() and client.request() exceptions", () => {
    for (const file of [
      ...
      expect(codingStandards, `docs must document exception: ${file}`).toContain(file);
    }
  });
});
