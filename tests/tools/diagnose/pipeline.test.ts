...
describe("pipelineHandler", () => {
  ...
  it("treats string return_download_url as URL mode for failed step logs", async () => {
    const { resolveLogContent, resolveLogDownloadUrl } = await import("../../../src/utils/log-resolver.js");
    (resolveLogContent as ReturnType<typeof vi.fn>).mockClear();
    (resolveLogDownloadUrl as ReturnType<typeof vi.fn>).mockClear();
    ...
    expect(resolveLogDownloadUrl).toHaveBeenCalledWith(expect.anything(), "log/step1", { signal: undefined });
    expect(resolveLogContent).not.toHaveBeenCalled();
  });
