...
vi.mock("../../src/utils/log-resolver.js", () => ({
  ...
  resolveLogDownloadUrl: vi.fn().mockResolvedValue("https://logs.example.com/download.zip"),
}));
...
describe("harness_get — execution_log", () => {
  ...
  beforeEach(async () => {
    ...
    resolveLogDownloadUrlMock.mockReset().mockResolvedValue("https://logs.example.com/download.zip");
  ...
  it("documents return_download_url in the registered input schema", () => {
    ...
    expect(schema.inputSchema.return_download_url?.description).toContain("signed download URL");
  });

  it("returns a signed download URL when schema-driven clients pass return_download_url", async () => {
    const rawArgs = {
      resource_type: "execution_log",
    ...
    const schemaFilteredArgs = Object.fromEntries(
      Object.entries(rawArgs).filter(([key]) => key in schema.inputSchema),
    );

    const result = await server.call("harness_get", schemaFilteredArgs);

    expect(result.isError).toBeUndefined();
    const data = parseResult(result) as { download_url: string };
    expect(data.download_url).toBe("https://logs.example.com/download.zip");
