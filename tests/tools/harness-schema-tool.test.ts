...
describe("harness_schema nested static definition lookup", () => {
  ...
  it("resolves newly synced IdentitiesConfig from v0 pipeline common definitions", async () => {
    const server = makeMcpServer();
    registerSchemaTool(server, undefined, undefined);
    const result = await server.call("harness_schema", {
      resource_type: "pipeline",
    ...
    const schema = parsed.schema as { maxProperties?: number };
    expect(schema.maxProperties).toBe(10);
  });

  it("resolves newly synced DynamicStageNodeV1 from v1 pipeline unified stages", async () => {
    const server = makeMcpServer();
    registerSchemaTool(server, undefined, undefined);
    const result = await server.call("harness_schema", {
      resource_type: "pipeline_v1",
