...
describe("harness_schema nested static definition lookup", () => {
  ...
  it("resolves newly synced ShiftAwsAgentCoreTraffic step definition from v0 pipeline", async () => {
    const server = makeMcpServer();
    registerSchemaTool(server, undefined, undefined);
    const result = await server.call("harness_schema", {
      resource_type: "pipeline",
      path: "ShiftAwsAgentCoreTrafficStepNode",
    });
    const parsed = parseResult(result) as Record<string, unknown>;

    expect(result.isError).toBeFalsy();
    expect(parsed.path).toBe("steps.cd.ShiftAwsAgentCoreTrafficStepNode");
    expect(parsed.requested_path).toBe("ShiftAwsAgentCoreTrafficStepNode");
    const schema = parsed.schema as { properties?: { type?: { enum?: string[] } } };
    expect(schema.properties?.type?.enum).toContain("ShiftAwsAgentCoreTraffic");
  });
