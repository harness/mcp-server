...
- **Correction (Sunil architecture follow-up)**: Writing back only when casing changed left whitespace-padded known values (e.g. `" Pending"`) untouched, so agents still got API 400s after a no-op canonicalize. Compare the fully normalized form to the raw string whenever any declared enum token matched; never trim/rewrite strings that matched no enum token; cover the public `harness_list` path, not just the helper; and avoid agent-facing copy that claims size must be passed "inside filters" when the top-level `size` arg also works.
