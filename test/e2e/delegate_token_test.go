...
// TestGetDelegateByTokenTools verifies that the get_delegate_by_token tool is available
func TestGetDelegateByTokenTools(t *testing.T) {
	...
	// Check that get_delegate_by_token tool is available
	var foundTool bool
	fmt.Println("Available delegate token tools:")
	for _, tool := range response.Tools {
		if tool.Name == "get_delegate_by_token" {
			foundTool = true
	...
	require.True(t, foundTool, "expected to find get_delegate_by_token tool")
}

// TestGetDelegateByToken verifies that the get_delegate_by_token tool works correctly
func TestGetDelegateByToken(t *testing.T) {
	...
	tokenName := "test_get_delegate_by_token_" + uuid.NewString()
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			getRequest.Params.Name = "get_delegate_by_token"
...
// TestGetDelegateByTokenValidation verifies input validation for get_delegate_by_token
func TestGetDelegateByTokenValidation(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			request.Params.Name = "get_delegate_by_token"
...
// TestGetDelegateByTokenWithScopes verifies get_delegate_by_token works with different scopes
func TestGetDelegateByTokenWithScopes(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			response, err := mcpClient.CallTool(ctx, request)
			require.NoError(t, err, "expected to call 'get_delegate_by_token' tool successfully")
