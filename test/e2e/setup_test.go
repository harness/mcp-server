...
import (
	...
	pkg "github.com/harness/mcp-server/common/pkg"
...
func setupMCPClient(t *testing.T, options ...clientOption) *mcpClient.Client {
	...
	setupClientOnce.Do(func() {
		...
		opts := &clientOpts{
			enabledToolsets: []string{"delegateTokens"},
		}
		...
		// Initialize toolsets
		tsg, err := tools.InitToolsets(context.Background(), cfg)
		if err != nil {
			setupClientErr = err
			return
		}
		...
		mcpServer := pkg.NewServer("0.0.1", cfg)
