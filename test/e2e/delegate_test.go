...
// TestListDelegatesToolAvailable verifies that the core_list_delegates tool is available
func TestListDelegatesToolAvailable(t *testing.T) {
	...
	// Check that core_list_delegates tool is available
	var foundTool bool
	for _, tool := range response.Tools {
		if tool.Name == "core_list_delegates" {
			foundTool = true
	...
	require.True(t, foundTool, "expected to find core_list_delegates tool")
}

// TestListDelegates verifies that the core_list_delegates tool works correctly
func TestListDelegates(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			request.Params.Name = "core_list_delegates"
...
// TestListDelegatesWithScopes verifies core_list_delegates works with different scopes
func TestListDelegatesWithScopes(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			response, err := mcpClient.CallTool(ctx, request)
			require.NoError(t, err, "expected to call 'core_list_delegates' tool successfully")
			require.False(t, response.IsError, "expected result not to be an error")
