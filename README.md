...
- **`fme_segment`** — `list`/`get`/`delete` are wired to the real `/fme/api/v4/segments` endpoint (consolidates `fme_standard_segment`/`fme_rule_based_segment`); `create` is not yet implemented (request body not yet confirmed).
