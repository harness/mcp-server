...
- Idle sessions are reaped after `MCP_SESSION_TTL_MS` milliseconds of inactivity (default 30 minutes). Actual expiry may be up to half the TTL later because the reaper runs at `min(60s, TTL/2)` intervals.
...
| `MCP_SESSION_TTL_MS`        | No       | `1800000`                   | HTTP transport only: idle session lifetime in milliseconds before the server reaps the session. Reaper runs every `min(60s, TTL/2)`                                                                                                                  |
...
216 resource types organized across 36 toolsets. Each resource type supports a subset of CRUD operations and optional execute actions.
...
| HTTP `Session not found...`                                                      | Session expired (`MCP_SESSION_TTL_MS` idle TTL, default 30 min) or already closed                    | Re-run `initialize` to create a new session, then retry with new header                                                              |
