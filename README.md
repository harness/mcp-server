...
**FME (Split.io) resources** — most `fme_`* resources are **Harness-native only**: every call requires `org_id`+`project_id` and hits Harness-native endpoints (standard `HARNESS_API_KEY`/`HARNESS_BASE_URL`, same auth as every other `harness_*` resource). Three resources — `fme_workspace`, `fme_identity`, `fme_segment_keys` — still support the deprecated legacy `workspace_id` (Split.io API, `api.split.io`) contract because no Harness-native equivalent exists for them yet.

- **`fme_workspace`** — no Harness-native equivalent; legacy-only (used to discover `workspace_id` values, still needed for `fme_identity`/`fme_segment_keys`).
- **`fme_environment`** — Harness-native only (`org_id`+`project_id` required; `/fme/api/v4/environments`). A stray `workspace_id` is silently ignored. `list` uses optional `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`); envelope `{data, limit, offset, totalCount}` is promoted to `items`/`total`. `create`/`update` use `isProduction` (`production` accepted as an alias). `update` is JSON Merge Patch; `name` and `isProduction` are not clearable. Name max 15 characters.
