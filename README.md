...
| `HARNESS_FME_API_KEY`       | No       | --                          | Optional FME/Split Admin API key used for `fme_` resources. FME calls go directly to `api.split.io`, so hosted OAuth/service-routing credentials for Harness platform APIs do not authenticate these requests. If unset, FME falls back to a non-placeholder `HARNESS_API_KEY` for self-hosted sessions |
