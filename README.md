...
| Resource Type                                | List | Get | Create | Update | Delete | Execute Actions                                         |
| -------------------------------------------- | ---- | --- | ------ | ------ | ------ | ------------------------------------------------------- |
| `feature_flag_workspace`                     | x    |     |        |        |        |                                                         |
| `feature_flag_environment`                   | x    | x   | x      | x      | x      |                                                         |
| `feature_flag`                               | x    | x   | x      | x      | x      | `kill`, `restore`, `reallocate`, `archive`, `unarchive` |
| `feature_flag_definition`                    | x    | x   | x      | x      | x      | `kill`, `restore`, `reallocate`                         |
| `feature_flag_rollout_status`                | x    |     |        |        |        |                                                         |
| `feature_flag_rule_based_segment`            | x    | x   | x      |        | x      |                                                         |
| `feature_flag_rule_based_segment_definition` | x    |     |        | x      |        | `enable`, `disable`, `change_request`                   |
| `feature_flag_traffic_type`                  | x    |     |        |        |        |                                                         |
| `feature_flag_identity`                      |      |     | x      | x      |        |                                                         |
| `feature_flag_standard_segment`              | x    | x   |        |        |        |                                                         |
| `feature_flag_segment`                       | x    | x   | x      |        | x      |                                                         |
| `feature_flag_segment_definition`            | x    | x   | x      | x      | x      |                                                         |
| `feature_flag_segment_keys`                  | x    |     |        | x      |        |                                                         |


**FME (Split.io) resources** — `feature_flag_`* resources support **dual-mode scoping**: legacy calls pass `workspace_id` and hit the Split.io API (`api.split.io`); newer calls pass `org_id`+`project_id` together and hit Harness-native endpoints (standard `HARNESS_API_KEY`/`HARNESS_BASE_URL`, same auth as every other `harness_*` resource) instead. Passing both `workspace_id` and `org_id`/`project_id` on the same call, or mixing `org_id` with `project_id` alone, is an error — pick one mode per call. Every operation below is available in legacy mode, unchanged. Harness-native mode coverage is currently narrower:

- **`feature_flag_workspace`** — no Harness-native equivalent; legacy-only (used to discover `workspace_id` values).
- **`feature_flag_environment`** — dual-mode `list` (`workspace_id` or `org_id`+`project_id`). `get`/`create`/`update`/`delete` are Harness-native only (`/fme/api/v4/environments`) — MCP never had a `workspace_id` contract for those ops. Native list uses optional `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`); envelope `{data, limit, offset, totalCount}` is promoted to `items`/`total`. Native create/update use `isProduction` (`production` accepted as an alias). Native update is JSON Merge Patch; `name` and `isProduction` are not clearable. Name max 15 characters.
- **`feature_flag`** — dual-mode, both branches fully wired. Harness-native (`org_id`+`project_id`): `list`/`get`/`create`/`delete` hit `/fme/api/v4/feature-flags` (body for `create`: `name`, `trafficType`, optional `description`/`tags`/`owners`, per `CreateFeatureFlagRequest`); `update` sends a merge-patch to `/fme/api/v4/feature-flags/{name}`; `archive`/`unarchive` hit `/fme/api/v4/feature-flags/{name}/archive|unarchive` (optional `comment` only — no `title`, per `ArchiveUnarchiveRequest`); `kill`/`restore`/`reallocate` hit `/fme/api/v4/feature-flag-definitions/{name}/kill|restore|reallocate` with `environment_id` as a query param (optional `comment`/`title`, per `FeatureFlagDefinitionActionRequest`).
- **`feature_flag_definition`** — `get`/`create`/`update` remain dual-mode (`workspace_id` or `org_id`+`project_id`). `list`/`delete`/`kill`/`restore`/`reallocate` are Harness-native only (`org_id`+`project_id`) — MCP never had a `workspace_id` contract for those ops. Native list requires `feature_flag_name` and uses `offset`/`limit` (default 100, max 100); it does not take `environment_id`. Delete and execute require `environment_id`. Kill/restore/reallocate are the same actions as on `feature_flag`. Get/create/update body matches legacy (`treatments`, `defaultTreatment`, `defaultRule`, optional `rules`/`baselineTreatment`/`trafficAllocation`/`comment`), plus optional `title` in Harness-native mode. Native update is JSON Merge Patch.
- **`feature_flag_rollout_status`** — dual-mode `list`. Pass `org_id`+`project_id` (preferred) or the deprecated `workspace_id`. Native pagination uses `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`); results are promoted to `items`/`total`. Each item has `id`, `name`, and optional `description`.

- **`feature_flag_rule_based_segment`** — (Deprecated — see `feature_flag_segment`.) Harness-native mode is rejected on every operation (`list`/`get`/`create`/`delete`) — use `feature_flag_segment` instead; this resource supports only the legacy `workspace_id` contract.
- **`feature_flag_rule_based_segment_definition`** — (Deprecated — see `feature_flag_segment_definition`.) Harness-native mode is rejected on every operation/action (`list`/`update`/`enable`/`disable`/`change_request`) — use `feature_flag_segment_definition` instead (no `enable`/`disable`/`change_request` equivalent there); this resource supports only the legacy `workspace_id`/`environment_id` contract.
- **`feature_flag_traffic_type`** — dual-mode `list`. Pass `org_id`+`project_id` (preferred) or the deprecated `workspace_id`. Native pagination uses `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`); results are promoted to `items`/`total`. Each item has `id` and `name` (no `displayAttributeId`).
- **`feature_flag_identity`** — `create`/`update` are not yet implemented if `org_id`+`project_id` are passed together; otherwise proceeds as a normal legacy call.
- **`feature_flag_standard_segment`** — (Deprecated — see `feature_flag_segment`.) Harness-native mode is rejected on every operation (`list`/`get`) — use `feature_flag_segment` instead; this resource supports only the legacy `workspace_id` contract. There is no `create` operation for this resource in either mode.
- **`feature_flag_segment_keys`** — `list`/`update` are not yet implemented if `org_id`+`project_id` are passed together; otherwise proceeds as a normal legacy call.
- **`feature_flag_segment`** — `list`/`get`/`create`/`delete` are wired to the real `/fme/api/v4/segments` endpoint (consolidates `feature_flag_standard_segment`/`feature_flag_rule_based_segment`); `create` body: `name`, `trafficType`, `type` (required — one of `standard`/`rule_based`/`large`), optional `description`/`tags`/`owners`.
- **`feature_flag_segment_definition`** — Harness-native only (no legacy `workspace_id` support). `list`/`get`/`create`/`update`/`delete` are wired to `/fme/api/v4/segment-definitions`, per `Harness_Split/Main` PR #12644 (open, not yet merged as of this writing — paths may still change). `update` uses JSON Merge Patch on `description`, the only mutable field. There is no `enable`/`disable`/`change_request` action — the backend has no such endpoints for this unified resource.

In single-user/self-hosted mode, legacy-mode auth uses a Bearer token from `HARNESS_FME_API_KEY`, falling back to a non-placeholder `HARNESS_API_KEY`. `HARNESS_FME_API_KEY` may be a legacy Split admin key or an FME-entitled Harness PAT/SAT, but it is rejected in `multi-user` mode so shared deployments cannot override each session user's credential. Hosted OAuth/service-routing credentials for Harness platform APIs do not authenticate direct Split.io requests. `feature_flag` supports full lifecycle management in legacy mode: create (requires `traffic_type_id`), list, get, update metadata, delete, and kill/restore/reallocate/archive/unarchive execute actions. Use `feature_flag_traffic_type` to discover traffic type IDs, `feature_flag_identity` to create/update identity attributes, and `feature_flag_standard_segment` / `feature_flag_segment_keys` to inspect standard segments and add member keys. `feature_flag_rule_based_segment` provides CRUD for targeting segments, while `feature_flag_rule_based_segment_definition` manages environment-specific segment rules with enable/disable and change request approval flows.
...
| `infrastructure`                    | infrastructure_workspace, infrastructure_variable_set, infrastructure_resource, infrastructure_module, infrastructure_provider, infrastructure_workspace_costs, infrastructure_activity_resource_change |
| `secrets`                           | secret |
| `logs`                              | execution_log |
| `audit`                             | audit_event |
...
| `templates`                         | template, template_v1 |
| `dashboards`                        | dashboard, dashboard_data |
| `developer_portal`                  | developer_portal_entity, scorecard, scorecard_check, scorecard_stats, scorecard_check_stats, developer_portal_score, developer_portal_score_summary, developer_portal_workflow, developer_portal_tech_doc |
| `pull_requests`                     | pull_request, pr_reviewer, pr_comment, pr_check, pr_activity |
| `feature_flags`                     | feature_flag_workspace, feature_flag_environment, feature_flag, feature_flag_definition, feature_flag_rollout_status, feature_flag_rule_based_segment, feature_flag_rule_based_segment_definition, feature_flag_traffic_type, feature_flag_identity, feature_flag_standard_segment, feature_flag_segment, feature_flag_segment_definition, feature_flag_segment_keys |
| `gitops`                            | gitops_agent, gitops_application, gitops_cluster, gitops_repository, gitops_applicationset, gitops_repo_credential, gitops_app_event, gitops_pod_log, gitops_managed_resource, gitops_resource_action, gitops_dashboard, gitops_app_resource_tree, gitops_cluster_link |
| `chaos`                             | chaos_experiment, chaos_experiment_run, chaos_probe, chaos_probe_in_run, chaos_experiment_template, chaos_experiment_variable, chaos_component_variable, chaos_input_set, chaos_infrastructure, chaos_loadtest, chaos_service, chaos_k8s_infrastructure, chaos_enabled_infrastructure, chaos_hub, chaos_fault, chaos_fault_experiment_run, chaos_fault_template, chaos_probe_template, chaos_action_template, chaos_action, chaos_hub_fault, chaos_environment, chaos_application_map, chaos_guard_condition, chaos_guard_rule, chaos_recommendation, chaos_risk, chaos_dr_test, discovered_agent, discovered_namespace, discovered_service, discovered_network_map, scanned_risk, chaos_risk_rule, chaos_risk_scan |
| `cost`                              | cost_perspective, cost_perspective_folder, cost_breakdown, cost_timeseries, cost_summary, cost_recommendation, cost_anomaly, cost_anomaly_drilldown, cost_anomaly_summary, cost_category, cost_budget, cost_budget_variance, cost_budget_group, cost_recommendation_filter, cost_account_overview, cost_filter_value, cost_recommendation_count, cost_recommendation_stats, cost_recommendation_detail, cost_commitment, unit_metric |
| `developer_insights`                | developer_insights_metric, developer_insights_productivity_metric, developer_insights_dora_metric, developer_insights_team, developer_insights_team_detail, developer_insights_org_tree, developer_insights_org_tree_detail, developer_insights_business_alignment, developer_insights_ai_usage, developer_insights_ai_adoption, developer_insights_ai_impact, developer_insights_ai_raw_metric |
| `application_security`              | application_security_issue, application_security_issue_filter, application_security_pipeline_issue, application_security_pipeline_step, application_security_exemption, application_security_exemption_bulk, application_security_remediation_diff, application_security_artifact_source, application_security_artifact, application_security_artifact_component, application_security_component_search, application_security_component_dependencies, application_security_artifact_remediation, application_security_chain_of_custody, application_security_compliance_result, application_security_bom_violation, application_security_code_repo, application_security_component_remediation, application_security_remediation_pr, application_security_auto_pr_config, application_security_component_enrichment, application_security_component_vulnerability, application_security_oss_risk_summary, application_security_project_overview, application_security_sbom_drift, application_security_component_drift, application_security_sbom |
| `evidence_vault`                    | attestation |
| `databases`                         | database_schema, database_instance, database_snapshot_object, database_changeset_existence, database_default_authoring_instance, database_llm_authoring_pipeline, database_execute_llm_authoring_pipeline |
...
| `ai_evals`                          | eval_dataset, eval_dataset_item, evaluation, eval_run, eval_run_item, eval_run_by_eval, eval_metric, eval_metric_set, eval_metric_set_entry, eval_suite, eval_suite_evaluation, eval_suite_run, eval_target, eval_annotation, online_eval, eval_analytics, eval_git_settings, eval_git_registration, eval_registry_item |
| `software_delivery_knowledge_graph` | software_delivery_knowledge_graph_queryable_type_summary, software_delivery_knowledge_graph_grammar, software_delivery_knowledge_graph_type, software_delivery_knowledge_graph_related_type, hql_query |
| `ansible` *(opt-in)*                | ansible_inventory, ansible_playbook, ansible_host, ansible_activity, ansible_host_activity |
| `incidents`                         | incident |
| `alerts`                            | alert |
| `deploys`                           | deploy |
| `release_management`                | release_process, release_activity, release, release_execution_phase, release_execution_task, release_execution_activity, release_execution_phase_output, release_execution_phase_input, release_execution_activity_output, release_input, release_execution_activity_input |
