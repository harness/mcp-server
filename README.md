...
- **`fme_metric`** — Harness-native only (no legacy `workspace_id` support). `list`/`get`/`create`/`update`/`delete`. List uses optional `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`); envelope `{data, limit, offset, totalCount}` is promoted to `items`/`total`. `create` body: `name`, `trafficType`, `format`, `aggregation`, `isPositive`, `baseEventTypes` (min 1), `owners` (min 1 — `{type: USER, id|email}` or `{type: GROUP, identifier}`), optional `description`/`spread`/`filterEventType`/`tags`/`cap`. `update` is JSON Merge Patch — omit fields to keep them, set `description`/`filterEventType`/`tags`/`owners`/`cap` to null to clear; `format`/`aggregation`/`isPositive`/`spread` cannot be null; `baseEventTypes` is a full replacement list (min 1) when provided. Use `metric_id` (UUID from `list` `items[].id` or `create` `entity.id`) for get/update/delete — not the metric name.
