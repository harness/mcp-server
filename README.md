...
- **`fme_rollout_status`** — dual-mode `list`. Legacy `workspace_id` still hits Split Admin `/internal/api/v2/rolloutStatuses/ws/{wsId}`. Harness-native (`org_id`+`project_id`) hits `GET /fme/api/v4/rollout-statuses` with optional `offset`/`limit` (max 100). Envelope `{data, limit, offset, totalCount}`; items `{type: "ROLLOUT_STATUS", id, name, description?}`. List-only.
...
- **`fme_traffic_type`** — dual-mode `list`. Legacy `workspace_id` still hits Split Admin `/internal/api/v2/trafficTypes/ws/{wsId}`. Harness-native (`org_id`+`project_id`) hits `GET /fme/api/v4/traffic-types` with optional `offset`/`limit` (max 100). Envelope `{data, limit, offset, totalCount}`; items `{type: "TRAFFIC_TYPE", id, name}` (no `displayAttributeId`). List-only.
