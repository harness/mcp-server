...
- **`fme_environment`** — dual-mode `list` (`workspace_id` or `org_id`+`project_id`). Harness-native list hits `GET /fme/api/v4/environments` with optional `offset`/`limit` (max 100; `harness_list` `size` maps to `limit`). Envelope `{data, limit, offset, totalCount}` is promoted to `items`/`total`. Get/create/update/delete are not wired yet.
...
- **`fme_segment`** — `list`/`get`/`create`/`delete` are wired to `/fme/api/v4/segments` (consolidates `fme_standard_segment`/`fme_rule_based_segment`). `list` requires `segment_type` (`STANDARD` | `LARGE` | `RULE_BASED`; case is canonicalized). `create` body: `name`, `trafficType`, `segmentType` (required — same kinds); optional `description`/`tags`/`owners`. Alias `type` (`standard` | `large` | `rule_based`) maps to `segmentType` for one release and is never sent on the wire.
