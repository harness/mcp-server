...
- **`fme_environment`** — dual-mode `list`/`create`/`update`/`delete`. Item `get` is Harness-native only (`GET /fme/api/v4/environments/{id}` — Split v2 has no GET-by-id). Native create/update use `isProduction` (`CreateEnvironmentRequest` / `UpdateEnvironmentRequest`); legacy uses `production`. Native update is JSON Merge Patch; `name` and `isProduction` are not clearable.
