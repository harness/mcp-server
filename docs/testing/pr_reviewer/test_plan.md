...
| TC-prv-003b | Create | Add a reviewer by email | `harness_create(resource_type="pr_reviewer", repo_id="my-repo", pr_number=1, body={"reviewer_email": "user@harness.io"})` | Adds the user as a reviewer |
| TC-prv-003c | Create | Add a reviewer by account user id | `harness_create(resource_type="pr_reviewer", repo_id="my-repo", pr_number=1, body={"reviewer_uid": "<user identifier>"})` | Adds the user as a reviewer |
...
| TC-prv-011 | Error | Create reviewer missing identity | `harness_create(resource_type="pr_reviewer", repo_id="my-repo", pr_number=1, body={})` | Returns error requiring reviewer_email or numeric reviewer_id |
...
- Prefer `reviewer_email` from `harness_list(user)`
- Numeric `reviewer_id` from `harness_list(pr_reviewer)` still works and wins if both email and id are set
- `harness_list(user)` identifier/uuid is not `reviewer_id`; pass `reviewer_email` or `reviewer_uid`
