...
| TC-fme_feature_flag-026b | Error | Create via Harness-native scope without trafficType | `harness_create(resource_type="fme_feature_flag", org_id="my_org", project_id="my_project", body={"name": "new_flag"})` | Error: `"trafficType" is required in Harness-native mode` |
| TC-fme_feature_flag-026c | Create | Create via Harness-native scope using top-level traffic_type_id | `harness_create(resource_type="fme_feature_flag", org_id="my_org", project_id="my_project", traffic_type_id="user", body={"name": "new_flag"})` | Routes to `POST /fme/api/v4/feature-flags` with `{name, trafficType}` — traffic type is in the body, not the path |
| TC-fme_feature_flag-027 | Update | Update via Harness-native scope | `harness_update(resource_type="fme_feature_flag", org_id="my_org", project_id="my_project", feature_flag_name="my_flag", body={"description": "x"})` | Routes to `PATCH /fme/api/v4/feature-flags/{name}` with merge-patch `Content-Type` and a partial object body |
| TC-fme_feature_flag-028 | Execute | Kill via Harness-native scope | `harness_execute(resource_type="fme_feature_flag", action="kill", org_id="my_org", project_id="my_project", feature_flag_name="my_flag", environment_id="env_1")` | Routes to `POST /fme/api/v4/feature-flag-definitions/{name}/kill` with `environment_id` query param |
| TC-fme_feature_flag-029 | Execute | Restore via Harness-native scope | `harness_execute(resource_type="fme_feature_flag", action="restore", org_id="my_org", project_id="my_project", feature_flag_name="my_flag", environment_id="env_1")` | Routes to `POST /fme/api/v4/feature-flag-definitions/{name}/restore` with `environment_id` query param |
| TC-fme_feature_flag-030 | Execute | Archive via Harness-native scope | `harness_execute(resource_type="fme_feature_flag", action="archive", org_id="my_org", project_id="my_project", feature_flag_name="my_flag")` | Routes to `POST /fme/api/v4/feature-flags/{name}/archive` |
| TC-fme_feature_flag-031 | Execute | Unarchive via Harness-native scope | `harness_execute(resource_type="fme_feature_flag", action="unarchive", org_id="my_org", project_id="my_project", feature_flag_name="my_flag")` | Routes to `POST /fme/api/v4/feature-flags/{name}/unarchive` |
...
- Harness-native list/get/create/delete path: `/fme/api/v4/feature-flags[/{featureFlagName}]`, scoped via `account_id`/`organization_identifier`/`project_identifier` query params (not the standard NG `orgIdentifier`/`projectIdentifier`). Create body shape (`name`, `trafficType`, optional `description`/`tags`/`owners`) is confirmed against the backend's `CreateFeatureFlagRequest` DTO (`Harness_Split/Main`), not guessed. Native create also accepts top-level `traffic_type_id` / `traffic_type` as aliases for `body.trafficType`.
- Environment is required for `kill`/`restore`/`reallocate`. In Harness-native mode those actions POST to `/fme/api/v4/feature-flag-definitions/{name}/kill|restore|reallocate` with `environment_id` as a query param. Archive/unarchive stay on `/fme/api/v4/feature-flags/{name}/archive|unarchive` and do not need an environment.
