...
| TC-einp-010 | Error | Missing execution ID | `harness_get(resource_type="execution_inputs")` | The tool call is accepted, then registry path-param resolution fails with `Missing required field "execution_id" for execution_inputs` because no `resource_id` or execution ID was provided |
