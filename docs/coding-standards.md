...
- Import or call `createLogger` (logging belongs in tool handlers and `src/utils/`, not toolsets)
...
| Import or call `createLogger` in a toolset file | Toolsets are pure data — log in handlers or utils only |
