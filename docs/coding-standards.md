...
- Import or call `createLogger` (logging belongs in tool handlers, not toolset definitions)
...
- Use `console.log()` or any `console.*` writes (stdio JSON-RPC safety)
...
| Import `createLogger` or use `console.*` in a toolset file | Logging belongs in handlers; toolsets must stay side-effect free |
