...
- Use `console.log()` / `console.*` or `createLogger()` (all logging belongs in tool handlers and registry code)
...
| Import `HarnessClient` or `createLogger` in a toolset file | Toolsets are pure data — they don't execute HTTP or write logs |
