...
- Use `console.log()` (or any stdout writes) or `createLogger()` — logging belongs in tool handlers and the registry
...
| Import `createLogger` in a toolset file | Logging belongs in handlers/registry — toolsets are pure data |
