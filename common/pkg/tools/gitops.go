...
- Agent type (managed vs hosted)`),
			...
			WithPagination(),
			mcp.WithString("type",
				mcp.Description("Optional: Filter by agent type"),
				mcp.Enum("MANAGED_ARGO_PROVIDER", "HOSTED_ARGO_PROVIDER"),
			),
...
- **History**: Deployment revision history`),
			...
			mcp.WithString("refresh",
				mcp.Description("Optional: Refresh mode - 'normal' refreshes if source changed, 'hard' forces manifest regeneration"),
				mcp.Enum("normal", "hard"),
			),
		),
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			scope, err := common.FetchScope(ctx, config, request, false)
