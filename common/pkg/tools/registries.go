...
func ListRegistriesTool(config *config.McpServerConfig, client *ar.ClientWithResponses) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("list_registries",
			...
			mcp.WithString("package_type",
				...
				mcp.Enum(string(ar.CARGO), string(ar.COMPOSER), string(ar.CONDA), string(ar.DART),
					string(ar.DOCKER), string(ar.GENERIC), string(ar.GO), string(ar.HELM),
					string(ar.HUGGINGFACE), string(ar.MAVEN), string(ar.NPM), string(ar.NUGET),
					string(ar.PYTHON), string(ar.RAW), string(ar.RPM), string(ar.SWIFT)),
			),
			WithPagination(),
			common.WithScope(config, false),
		),
