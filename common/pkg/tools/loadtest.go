...
func CreateSampleLoadTestTool(config *config.McpServerConfig, client *client.LoadTestService) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("chaos_create_sample_loadtest",
			mcp.WithDescription("Create a sample load test. Requires a name and a load runner infrastructure ID (use chaos_list_linux_infrastructures to find available infrastructure)."),
			common.WithScope(config, false),
			mcp.WithString("name",
			...
			mcp.WithString("locust_cluster_id",
				mcp.Description("The infrastructure ID of the load runner to use (get from chaos_list_linux_infrastructures)"),
				mcp.Required(),
			),
		),
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
