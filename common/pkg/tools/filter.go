...
func BuildArtifactComponentViewBody(request mcp.CallToolRequest) (generated.ArtifactComponentViewRequestBody, error) {
	...
	slog.Info("BuildArtifactComponentViewBody: raw component_filter", "rawFilter", rawFilter, "type", fmt.Sprintf("%T", rawFilter))

	if err := bindParam(request, "component_filter", func(v []generated.ComponentFilter) {
		slog.Info("BuildArtifactComponentViewBody: parsed component_filter", "filter", v, "len", len(v))
