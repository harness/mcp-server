...
func runMCPServer(ctx context.Context, config config.Config) error {
	...
	if config.Transport == "http" {
		...
		return runStdioServerWithServer(ctx, harnessServer, config)
	}
}
...
// runStdioServerWithServer is the internal function that handles stdio transport with an existing server
func runStdioServerWithServer(ctx context.Context, harnessServer *server.MCPServer, config config.Config) error {
	// Create stdio server
	stdioServer := server.NewStdioServer(harnessServer)
	...
	// Output startup message
	slog.Info("Harness MCP Server running on stdio (runStdioServerWithServer)", "version", version)
